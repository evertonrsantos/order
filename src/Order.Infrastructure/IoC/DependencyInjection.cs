...
namespace Order.Infrastructure.IoC
{
    public static class DependencyInjection
    {
        ...
        {
            // MediatR
            services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(RegisterOrderCommand).Assembly));            // FluentValidation
            services.AddValidatorsFromAssembly(typeof(RegisterOrderValidator).Assembly);
            
            // Use the built-in validation pipeline behavior
            services.AddTransient(typeof(IPipelineBehavior<,>), 
                typeof(FluentValidation.MediatR.PipelineBehaviors.ValidationBehavior<,>));
